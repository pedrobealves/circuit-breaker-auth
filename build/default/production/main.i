...
void __attribute__((picinterrupt(("high_priority")))) high_isr(void) {
    ...
    if (INTCONbits.RBIF == 1){
     if (PORTBbits.RB4 == 0) row = 1;
     if (PORTBbits.RB5 == 0) row = 2;
     if (PORTBbits.RB6 == 0) row = 3;
     if (PORTBbits.RB7 == 0) row = 4;
     INTCONbits.RBIF = 0;
   }
}
...
void show_status_relay(){
    ...
    do{
        ADCON0bits.GO_DONE = 1;
        if ((0 ? isdigit(key) : ((unsigned)(key)-'0') < 10) || flag_status_relay){
...
void change_password(){
    ...
    clean_lcd();
    WriteCmdXLCD(0x82);
    putsXLCD ("SENHA ATUAL: ");
    char current_password[5];
    get_password(current_password);
    _delay((unsigned long)((500)*(20000000/4000.0)));
    if ((grant_access(current_password) - '0') > 0) {
        clean_lcd();
        WriteCmdXLCD(0x83);
        putsXLCD ("NOVA SENHA: ");
        char new_password[5];
...
void update_password(const char* current_password, const char* new_password) {
    ...
    while (address < 200) {
        ...
        if (strcmp(current_password, user.password) == 0) {
            ...
            EEPROM_write_user(&user, address);
            return;
        }
...
void assign_default_passwords() {
    ...
    if (!has_passwords()) {
        EEPROM_write_user(&user1, 0);
        EEPROM_write_user(&user2, 6);
        EEPROM_write_user(&user3, 12);
    }
}
